...
from structured_segmentation.learner.internal_regressor import RegressorHandler
from structured_segmentation.learner.internal_classifier import InternalClassifier
from layers.layer_operations import resize, dropout, augment_tag
...
class ShapeRefinement3DLayer:
    ...
    def __init__(self,
        ...
        if clf_options is None:
            ...
            clf_opt["type"] = clf
        if shape == "arbitrary":
            self.clf = InternalClassifier(opt=clf_opt)
            self.clf.new()
        else:
