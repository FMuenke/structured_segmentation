...
class Input2DPlusTimeLayer:
    layer_type = "INPUT2D_PLUS_TIME_LAYER"

    def __init__(self, name, features_to_use, frame_range, height=None, width=None, initial_down_scale=None):
        self.name = name
    ...
    def __str__(self):
        return "{}-{}-{}-Frames-{}".format(self.layer_type, self.name, self.features_to_use, self.frame_range)
    ...
    def inference(self, data_3d, interpolation="nearest"):
        x = []
        for image in data_3d:
            xi = self.inference_2d(image)
            x.append(xi)
        return np.concatenate(x, axis=2)

    def inference_2d(self, image, interpolation="nearest"):
